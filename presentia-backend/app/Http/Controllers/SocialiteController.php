...
class SocialiteController extends Controller
{
    ...
    public function googleAuthentication() 
    {
        ...
        try {
            ...
            if (!$user) {
                return redirect(config('app.frontend_url') . '/login?status=new_user&name=' . urlencode($googleUser->name) . '&email=' . urlencode($googleUser->email) . '&google_id=' . urlencode($googleUser->id));
            }
